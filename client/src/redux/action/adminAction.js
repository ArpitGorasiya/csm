...
const adminAddNoticeFlag = (data) => {
    return {
        type: "ADMIN_ADD_NOTICE_FLAG",
        payload: data
    }
}
...
export const adminGetAllNotice = () => {
    return async (dispatch) => {
        try {
            const { data } = await axios({
                method: 'Get',
                url: url + "/api/admin/getAllNotice",
            })
            dispatch(getAllNotice(data))
        }
        catch (err) {
            console.log("Error in getting all notice", err.message)
        }
    }
}
...
export const adminAddNotice = (noticeCredential) => {
    return async (dispatch) => {
        try {
            const { data } = await axios({
                method: 'Post',
                url: url + "/api/admin/addNotice",
                data: noticeCredential
            })
            dispatch(adminAddNoticeFlag(true))
